...
- The bot trades the same option side as the source alert: source `CE` opens `CE`, and source `PE` opens `PE`.
...
- Keeps the extracted option side before entry: `CE -> CE`, `PE -> PE`.
